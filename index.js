...
import { Lensflare, LensflareElement } from 'https://cdn.skypack.dev/three@0.132.2/examples/jsm/objects/Lensflare.js'
...
var cycleSpeed = {} 
cycleSpeed.value = 60 // 1 Earth cycle & 1/27.322 Moon cycles in n seconds; 86160 is exactly one full day
var rotationVelocity = 2 * Math.PI / cycleSpeed.value
var sunCycle = 2 * Math.PI / cycleSpeed.value / 365.24
var earthCycle = 2 * Math.PI / cycleSpeed.value
...
earthFolder.add(cycleSpeed, 'value', 0, 120)
earthFolder.open()
gui.close()

function animate(t) {
  /*
  sunCoordX = Math.cos(sunCoordRadians) * sunCoordRadius
  sunCoordY = -Math.sin(sunCoordRadians) * sunCoordRadius
  sunMesh.position.set(sunCoordX, 0, sunCoordY)
  */
